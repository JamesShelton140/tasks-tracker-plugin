...
public class TaskService
{
	...
	private HashMap<String, TaskType> cachedTaskTypes = new HashMap<>();
	...
	public CompletableFuture<Boolean> setTaskType(TaskType newTaskType)
	{
		...
		configManager.setConfiguration(TasksTrackerPlugin.CONFIG_GROUP_NAME, "taskTypeJsonName",
		...
		return newTaskType.loadTaskTypeDataAsync().thenCompose((isTaskTypeLoaded) -> {
			...
			currentTaskType.getIntParamMap().keySet().forEach(paramName -> {
				...
				addSortedIndex(paramName,
					Comparator.comparingInt((TaskFromStruct task) -> task.getIntParam(paramName)));
			});
			...
			if (tasks.stream().anyMatch(task -> task.getCompletionPercent() != null))
			{
				...
				addSortedIndex("completion %", (TaskFromStruct task1, TaskFromStruct task2) -> {
					Float comp1 = task1.getTaskDefinition().getCompletionPercent() != null
						? task1.getTaskDefinition().getCompletionPercent()
						: 0;
					Float comp2 = task2.getTaskDefinition().getCompletionPercent() != null
						? task2.getTaskDefinition().getCompletionPercent()
						: 0;
