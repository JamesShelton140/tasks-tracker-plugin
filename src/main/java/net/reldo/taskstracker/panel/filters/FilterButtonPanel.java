...
public abstract class FilterButtonPanel extends FilterPanel
{
	...
	private final BufferedImage collapseImg = ImageUtil.loadImageResource(TasksTrackerPlugin.class,
		expandBtnPath + "filter_buttons_collapsed.png");
	private final Icon menuCollapsedIcon = new ImageIcon(ImageUtil.alphaOffset(collapseImg, -180));
	private final Icon menuIconHover = new ImageIcon(collapseImg);
	private final BufferedImage expandedImg = ImageUtil.loadImageResource(TasksTrackerPlugin.class,
		expandBtnPath + "filter_buttons_expanded.png");
	private final Icon menuExpandedIcon = new ImageIcon(ImageUtil.alphaOffset(expandedImg, -180));
	private final Icon menuIconHoverSelected = new ImageIcon(expandedImg);
	...
	protected JToggleButton makeButton(String tooltip, BufferedImage image)
	{
		...
		button.setBorder(
			new BasicBorders.ToggleButtonBorder(ColorScheme.DARKER_GRAY_COLOR, ColorScheme.DARKER_GRAY_COLOR.darker(),
	...
	public JToggleButton makeCollapseButton()
	{
		...
		collapseBtn.setIcon(menuCollapsedIcon);
		collapseBtn.setSelectedIcon(menuExpandedIcon);
		collapseBtn.setRolloverIcon(menuIconHover);
		collapseBtn.setRolloverSelectedIcon(menuIconHoverSelected);
	...
	{
		collapseBtn.setText(label + " - " + buttons.values().stream().filter(JToggleButton::isSelected).count() + " / "
			+ buttons.size());
	}
